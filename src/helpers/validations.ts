export const validationsForm = (form:any) => {
  let errors:any =  {};
  let regexUser:RegExp = /^[a-zA-Z0-9]([._-](?![._-])|[a-zA-Z0-9]){3,18}[a-zA-Z0-9]$/;
  let regexEmail:RegExp = /^(\w+[/./-]?){1,}@[a-z]+[/.]\w{2,}$/;
  let regexPassword:RegExp = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z]{8,}$/;
  let regexName:RegExp = /^[A-Za-zÑñÁáÉéÍíÓóÚúÜü\s]+$/;
